      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
        {fetching ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">No attendance records yet</p>
            <p className="text-xs text-slate-400 mt-1">Add manual attendance using the button above</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-100 dark:border-slate-700">
                  ...
                  <th className="text-left px-4 py-3 font-semibold text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wide">Event / Class</th>
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-700">
                {records.slice(0, 100).map((rec: any) => (
                  <tr key={rec.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition">
                    ...
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300 text-sm">{rec.recording?.month?.class?.name || rec.eventName || '—'}</td>
